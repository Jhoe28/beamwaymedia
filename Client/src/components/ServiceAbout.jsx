...
const ServiceAbout = ({ headline, about, title, accentImg, listItem1, listItem2, listItem3, listItem4, listItem5, listItem6, ourApproach, className }) => {
  ...
  return (
    <div className={`flex max-md:flex-col max-md:gap-20 items-center bg-gray-200 justify-between py-15 md:py-20 px-4 md:px-16 lg:px-24 xl:px-32 ${className || 'md:flex-row'}`}>
      <div className="flex flex-col items-start md:pr-16">
        <h2 data-delay="200" className='reveal text-3xl font-bold md:text-5xl leading-15'>{headline}</h2>
        <p data-delay="300" className="reveal text-left text-sm text-slate-700 max-w-lg mt-4">
          {about}
        </p>
        <ul className="mt-6 space-y-2 px-6">
          {listItems.map((item, index) => (
            <li data-delay="400" key={index} className="reveal flex items-center gap-2 text-sm text-slate-700" style={{ transitionDelay: `${index * 0.1}s`}}>
