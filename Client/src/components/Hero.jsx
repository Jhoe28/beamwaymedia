...
const Hero = (props) => {
  return (
    <div className={`flex flex-col max-md:gap-6 md:flex-row items-center justify-between py-12 md:py-16 mt-0 px-4 md:px-16 lg:px-24 xl:px-32 ${props.className || ''}`}>
        <div className={`flex flex-col gap-3 h-full my-4 ${props.contentClassName || 'items-start'}`}>
            ...
            {props.ctaText && (
                <div className="flex items-center gap-4 mt-0 md:mt-8 text-sm animate-heroAnimation">
                    <Link 
                        to={props.href || '/'}
